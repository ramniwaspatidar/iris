...
{
    __weak RevealViewController *_c;
}
...
#pragma mark - RevealViewController Class
...
- (void)prepareForSegue:(RevealViewControllerSegue *)segue sender:(id)sender
{
    // $ using a custom segue we can get access to the storyboard-loaded rear/front view controllers
    // the trick is to define segues of type RevealViewControllerSegue on the storyboard
    // connecting the RevealViewController to the desired front/rear controllers,
    ...
    // was used to instantiate the RevealViewController
    
    // $ none of this would be necessary if Apple exposed "relationship" segues for container view controllers.
    
    NSString *identifier = segue.identifier;
    if ( [segue isKindOfClass:[RevealViewControllerSegue class]] && sender == nil )
    {
        if ( [identifier isEqualToString:SWSegueRearIdentifier] )
        {
            segue.performBlock = ^(RevealViewControllerSegue* rvc_segue, UIViewController* svc, UIViewController* dvc)
            {
...
- (void)loadView
{
    ...
    self.lockView.alpha = 0.4;
    self.lockView.translatesAutoresizingMaskIntoConstraints = NO;
    self.lockView.backgroundColor = [UIColor blackColor];
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(lockViewClicked:)];
    [self.lockView addGestureRecognizer:tap];
    [self.lockView setTag:1000];
...
- (void)setFrontViewPosition:(FrontViewPosition)frontViewPosition animated:(BOOL)animated
{
    ...
    {
        [self.frontViewController.view addSubview:self.lockView];
        
        [self.frontViewController.view addConstraint:[NSLayoutConstraint constraintWithItem:self.lockView
                                                                                  attribute:NSLayoutAttributeTop
                                                                                  relatedBy:NSLayoutRelationEqual
                                                                                     toItem:self.lockView.superview
                                                                                  attribute:NSLayoutAttributeTop
                                                                                 multiplier:1.0
                                                                                   constant:0.0]];
...
- (void)revealController:(RevealViewController *)revealController
...
- (void)revealToggle:(id)sender
{
    ...
    [[ConnectionManager sharedInstance] hideMBHUDLoader];
...
- (void)_dispatchSetFrontViewPosition:(FrontViewPosition)frontViewPosition animated:(BOOL)animated
{
    ...
    __weak RevealViewController *theSelf = self;
...
- (void)_dispatchSetFrontViewController:(UIViewController *)newFrontViewController animated:(BOOL)animated
{
    ...
    // int initialPosDif = abs( _frontViewPosition - preReplacementPosition ); change by ram
...
- (void)lockViewClicked:(UITapGestureRecognizer *)gestureRecognizer{
    
    [self revealToggleAnimated:YES];
    
    //[self revealToggleAnimatedWhenLockViewCliked:YES];
...
-(void)rightSwipeHandler:(UISwipeGestureRecognizer *)recognizer
{
    ...
    if (self.frontViewPosition == FrontViewPositionLeft){
...
-(void)leftSwipeHandler:(UISwipeGestureRecognizer *)recognizer
{
    ...
    //    if ([self.frontViewController  isKindOfClass:[SplitVC class]])
    //    {
    //        ThreeTabViewController tvc = (ThreeTabViewController )[[[[self.frontViewController childViewControllers]lastObject]childViewControllers]lastObject];
    //        if (tvc.tabBarController)
    //        {
    //            UINavigationController *nav = tvc.tabBarController.selectedViewController;
    //            NSArray *arr = [nav childViewControllers];
    //            if ([[arr lastObject] isKindOfClass:[FullImageViewController class]])
    //            {
    //                return;
    //
    //            }
...
- (RevealViewController*)revealViewController
{
    ...
    Class revealClass = [RevealViewController class];
...
#pragma mark - RevealViewControllerSegue Class

@implementation RevealViewControllerSegue
