...
@property (nonatomic, strong) NSMutableArray* layers1;
@property (nonatomic, strong) NSMutableArray* layers2;
@property (nonatomic, strong) NSMutableArray* layers3;
@property (nonatomic, strong) NSMutableArray* layers4;
@property (nonatomic, strong) NSMutableArray* layers5;
...
@implementation FSMutipleLineChart
...
- (void)commonInit
{
    ...
    _layers1 = [NSMutableArray array];
    _layers2 = [NSMutableArray array];
    _layers3 = [NSMutableArray array];
    _layers4 = [NSMutableArray array];
    _layers5 = [NSMutableArray array];
...
- (void)setDefaultParameters
{
    ...
    _indexLabelTextColor = [UIColor blackColor];
...
- (void)layoutSubviews
{
    ...
    {
        [self.layers1 enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            CALayer* layer = (CALayer*)obj;
            [layer removeFromSuperlayer];
        }];
    }
    ...
    {
        [self.layers2 enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            CALayer* layer = (CALayer*)obj;
            [layer removeFromSuperlayer];
        }];
    }
    else if(self.currentTag == 4)
    {
        [self.layers3 enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            CALayer* layer = (CALayer*)obj;
            [layer removeFromSuperlayer];
        }];
    }
    else if(self.currentTag == 5)
    {
        [self.layers4 enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            CALayer* layer = (CALayer*)obj;
            [layer removeFromSuperlayer];
        }];
    }
    ...
    {
        [self.layers5 enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            CALayer* layer = (CALayer*)obj;
            [layer removeFromSuperlayer];
        }];
    }
...
- (void)setChartData:(NSArray *)chartData withTag:(int)tag
{
    ...
    {
        _data1 = [NSMutableArray arrayWithArray:chartData];
        self.currentTag = 2;
    }
    else if(tag == 3)
    {
        _data2 = [NSMutableArray arrayWithArray:chartData];
        self.currentTag = 3;
    }
    else if(tag == 4)
    {
        _data3 = [NSMutableArray arrayWithArray:chartData];
        self.currentTag = 4;
    }
    else if(tag == 5)
    {
        _data4 = [NSMutableArray arrayWithArray:chartData];
        self.currentTag = 5;
    }
    else if(tag == 6)
    {
        _data5 = [NSMutableArray arrayWithArray:chartData];
        self.currentTag = 6;
    }
...
- (UILabel*)createLabelForValue: (NSUInteger)index
{
    ...
    //NSString* text = [self.leftArray objectAtIndex:index];
    ...
    label.textColor = [UIColor blackColor];//_valueLabelTextColor;
    label.textAlignment = NSTextAlignmentCenter;
    label.backgroundColor = [UIColor clearColor];//_valueLabelBackgroundColor;
...
- (void)clearChartData
{
    ...
    for (CAShapeLayer *layer in self.layers1) {
        [layer removeFromSuperlayer];
    }
    [self.layers1 removeAllObjects];
    
    for (CAShapeLayer *layer in self.layers2) {
        [layer removeFromSuperlayer];
    }
    [self.layers2 removeAllObjects];
    
    for (CAShapeLayer *layer in self.layers3) {
        [layer removeFromSuperlayer];
    }
    [self.layers3 removeAllObjects];
    
    for (CAShapeLayer *layer in self.layers4) {
        [layer removeFromSuperlayer];
    }
    [self.layers4 removeAllObjects];
    
    for (CAShapeLayer *layer in self.layers5) {
        [layer removeFromSuperlayer];
    }
    [self.layers5 removeAllObjects];
    
    
    [self.data5 removeAllObjects];
...
- (void)strokeChart
{
    ...
    if(_fillColor) {
        ...
        if(self.currentTag == 1)
            ...
            [self.layers1 addObject:fillLayer];
        else if(self.currentTag == 3)
            [self.layers2 addObject:fillLayer];
        else if(self.currentTag == 4)
            [self.layers3 addObject:fillLayer];
        else if(self.currentTag == 5)
            [self.layers4 addObject:fillLayer];
        else if(self.currentTag == 6)
            [self.layers5 addObject:fillLayer];
    ...
    if(self.currentTag == 1)
        ...
        [self.layers1 addObject:pathLayer];
    else if(self.currentTag == 3)
        [self.layers2 addObject:pathLayer];
    else if(self.currentTag == 4)
        [self.layers3 addObject:pathLayer];
    else if(self.currentTag == 5)
        [self.layers4 addObject:pathLayer];
    else if(self.currentTag == 6)
        [self.layers5 addObject:pathLayer];
...
- (void)strokeDataPoints
{
    ...
    if(self.currentTag == 1)
       currentArray = [_data copy];
    else if(self.currentTag == 2)
        currentArray = [_data1 copy];
    else if(self.currentTag == 3)
        currentArray = [_data2 copy];
    else if(self.currentTag == 4)
        currentArray = [_data3 copy];
    else if(self.currentTag == 5)
        currentArray = [_data4 copy];
    else if(self.currentTag == 6)
        currentArray = [_data5 copy];

    for(int i=0;i<currentArray.count;i++) {
        
        int currentData = [[currentArray objectAtIndex:i] intValue];
        if(currentData != 0)
        {
...
- (void)computeBounds
{
    ...
    for(int i=0;i<_data5.count;i++) {
        NSNumber* number = _data5[i];
        if([number floatValue] < _min)
            _min = [number floatValue];
...
- (UIBezierPath*)getLinePath:(float)scale withSmoothing:(BOOL)smoothed close:(BOOL)closed
{
    ...
    if(smoothed) {
        for(int i=0;i<_data.count - 1;i++) {
            ...
            if(i == 0)
                [path moveToPoint:p];
            int currentData = [[_data objectAtIndex:i] intValue];
            if(currentData != 0)
            {
    ...
    if(closed) {
        ...
        [path addLineToPoint:[self getPointForIndex:currentArray.count - 1 withScale:scale]];
        [path addLineToPoint:[self getPointForIndex:currentArray.count - 1 withScale:0]];
