...
- (UILabel*)createLabelForValue: (NSUInteger)index
{
    ...
    if([self.chartType isEqualToString:@"BMI"])
    {
        NSArray *steps = @[@200,@264,@314,@515];
        float yAxis = _axisHeight + _margin - (_axisHeight/500)* [[steps objectAtIndex:index] intValue];
        p = CGPointMake(_margin + (_valueLabelPosition == ValueLabelRight ? _axisWidth : 0), yAxis);
        /*
        if(index == 0)
            p = CGPointMake(_margin + (_valueLabelPosition == ValueLabelRight ? _axisWidth : 0), _axisHeight + _margin - (index + 1) * _axisHeight / (_verticalGridStep - 1));
        else if(index == 1)
        {
            p =  CGPointMake(_margin + (_valueLabelPosition == ValueLabelRight ? _axisWidth : 0), _axisHeight- 10 + _margin - (index + 1) * _axisHeight / _verticalGridStep);
        }
        else if(index == 2)
        {
            p =  CGPointMake(_margin + (_valueLabelPosition == ValueLabelRight ? _axisWidth : 0), _axisHeight+ 10 + _margin - (index + 1) * _axisHeight / _verticalGridStep);
         ...
         */
        text = [self.leftArray objectAtIndex:index];
    }
    else
    {
        p = CGPointMake(_margin + (_valueLabelPosition == ValueLabelRight ? _axisWidth : 0), _axisHeight + _margin - (index + 1) * _axisHeight / _verticalGridStep);
        
        text = _labelForValue(minBound + (maxBound - minBound) / _verticalGridStep * (index + 1));
